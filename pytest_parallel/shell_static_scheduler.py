...
def submit_items(items_to_run, SCHEDULER_IP_ADDRESS, port, main_invoke_params, ntasks, i_step, n_step):
    ...
    for i,item in enumerate(items):
        ...
        cmd += f' ; python -m pytest_parallel.send_report {socket_flags} --_test_idx={test_idx}' # ';' : executes what is after, whether the command succeded or failed
        #                                                                     # '$?': return code of last command (that is, the test)
        cmds.append(cmd)
