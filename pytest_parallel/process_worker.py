...
class ProcessWorker:
    ...
    def _file_path(self, when):
        return Path(f'.pytest_parallel/{self.test_idx}_{when}')
    ...
    def pytest_runtestloop(self, session) -> bool:
        ...
        ## Setup a handler to send a message if a signal was raised
        #def send_error_signal_handler(sig, frame):
        #  print(f'\n\n\n{item.test_info=}, SIG={sig}')
        #  with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        #      s.connect((self.scheduler_ip_address, self.scheduler_port))
        #      item.test_info.update({'signal_info': sig})
        #      socket_utils.send(s, pickle.dumps(item.test_info))
        #      sys.exit(128+sig)
        #catchable_sigs = set(signal.Signals) - {signal.SIGKILL, signal.SIGSTOP}
        #for sig in catchable_sigs:
        #    signal.signal(sig, send_error_signal_handler)
